...
describe("MemberVote contract", function () {
  ...
  it("Deploy MemberVote Contract", async function () {
    ...
    cartContract = new ethers.Contract(
      ownerContract.address,
      artifact.abi,
      Cart
    );
  ...
  it("Contribute to the club", async function () {
    ...
    expect((await memberContract.shares(Bob.address)).toString()).to.equal(
      "101"
    );
  });

  it("You can't redeem more than your share", async function () {
    await expect(memberContract.redeemShare(200)).to.be.revertedWith(
      "not enough shares"
    );
  });
  ...
  it("You can't transfer shares more than your share", async function () {
    await expect(
      memberContract.transferShare(200, Cart.address)
  ...
  it("Create a proposal with too much amount will fail", async function () {
    await expect(
      memberContract.createProposal("Buy toys", 200, Cart.address)
